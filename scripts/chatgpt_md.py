...
def html_to_markdown(source: str) -> str:
    ...
    parser.feed(source)
    parser.close()
    return clean_markdown("".join(parser.out))
...
def _normalize_latex_delimiters(segment: str) -> str:
    """Convert ChatGPT's TeX delimiters to widely supported Markdown math."""
    segment = re.sub(
        r"\\\[(.*?)\\\]",
        lambda match: "\n$$\n" + match.group(1).strip() + "\n$$\n",
        segment,
...
def _clean_noncode(segment: str) -> str:
    segment = segment.replace("\r\n", "\n").replace("\r", "\n")
    segment = segment.replace("\xa0", " ").replace("\u200b", "").replace("\ufeff", "")
    segment = _normalize_latex_delimiters(segment)
    lines = segment.split("\n")
    cleaned: list[str] = []
...
def clean_markdown(source: str) -> str:
    ...
    for index, part in enumerate(parts):
        if index % 2:
            ...
            output.append(_clean_noncode(part))
...
def main() -> int:
    ...
    try:
        ...
        result = html_to_markdown(source) if is_html else clean_markdown(source)
        if args.stdout:
